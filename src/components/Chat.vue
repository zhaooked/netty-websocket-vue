...
export default {
  ...
  watch:{
    ...
    "$store.state.realchat.chatinfolist":function(){
      ...
      let chatList = $.extend(true, [], this.data.offlinelist)
      let defaultMessage = {
            "avatar": "",
            "time":"",
            "is_type": "isOther",
            "meType": chatText.meType
          }
          if (chatText.type === 'text_message') {
            defaultMessage.text = chatText.content
          } else if (chatText.type === 'img_message') {
            defaultMessage.url = chatText.url
          }
      let nowtime = this.time(chatText.timestamp,'qt');
  ...
  methods: {
    ...
    formChatList:function(chatText,chatList,defaultMessage){   // 从实时聊天过来
      ...
      if(chatList[0]){ // 如果聊天存在
        for(var i in chatList){
          if(chatList[i].aid != chatText.aid){
        ...
        if(blist[0]){ // 如果存在 则说明 存在当前聊天列表中
          ...
          blist[0].last_message = chatText.type === 'text_message' ? chatText.content : '图片'
          ...
          this.$fetch(this.getfriendInfo,{sessionId: chatText.aid}).then((response) => {
              var newData = response
              defaultMessage.avatar = newData.avatar
              newData.message_count = 1
              newData.last_message = chatText.type === 'text_message' ? chatText.content : '图片'
        ...
        this.$fetch(this.getfriendInfo,{sessionId: chatText.aid}).then((response) => {
            ...
            defaultMessage.avatar = newData.avatar
            newData.message_count = 1
            newData.session_id = chatText.aid
            newData.last_message = chatText.type === 'text_message' ? chatText.content : '图片'
            newData.last_send_at = this.time(chatText.timestamp,'hm')
            newData.messages = []
            newData.current = ''
    ...
    fromFriendsList:function(selectData,chatList){
      ...
      if(selectData[0]){ // 如果有从好友列表过来的才执行操作
        for(var i in chatList){
          if(chatList[i].aid != selectData[0].aid){
            chatList[i].current = ''
            alist.push(chatList[i])  //  好友列表中无
          }else{
            chatList[i].current = ''
